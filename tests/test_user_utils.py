...
def test_get_user(test_db):
    user_id = 2  # Set this to an existing user ID
    user = get_user(test_db, user_id)
    assert user is not None
    assert user.id == user_id

def test_get_user_by_email(test_db):
    email = "demo@radai.com"  # Set this to an existing user email
...
def test_create_user(test_db):
    user_data = UserCreate(email=f"newuser{randint(1,10000)}@example.com", password="newuserpassword", is_active=True)
